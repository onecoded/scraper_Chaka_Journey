...
scrape_bizquest.py
------------------
Scrapes business-for-sale listings from BizQuest.com.
BizQuest uses static HTML similar to BizBuySell (same parent company).

URL pattern: https://www.bizquest.com/businesses-for-sale/{state-name}/

IMPORTANT: If selectors stop working, inspect live HTML and update SELECTORS below.
Document selector changes in workflows/scrape_listings.md.
...
SELECTORS = {
    "listing_card":     "div.srp-listing-card, div.listing-card, article.listing",
    "title":            "h3 a, h2 a, .listing-card__title a, .srp-listing-card__title a",
    "price":            ".srp-listing-card__price, .listing-price, .price",
    "location":         ".srp-listing-card__location, .location, .listing-location",
    "category":         ".srp-listing-card__category, .category, .listing-category",
    "description":      ".srp-listing-card__description, .description, .listing-description",
    "pagination_next":  "a[rel='next'], .pagination .next a, a.next",
}
...
def scrape_state(state_code: str, max_pages: int) -> list[dict]:
    ...
    for page in range(1, max_pages + 1):
        if page == 1:
            urls.append(f"{BASE_URL}/businesses-for-sale/{state_slug}/")
        else:
            urls.append(f"{BASE_URL}/businesses-for-sale/{state_slug}/?page={page}")
    ...
    for url, html in page_results:
        ...
        if not cards:
            debug_path = Path(".tmp") / f"debug_bq_{state_code}.html"
...
def main():
    parser = argparse.ArgumentParser(description="Scrape BizQuest business listings")
    parser.add_argument("--states", nargs="+", default=["FL", "TX", "GA", "NC", "SC"])
    parser.add_argument("--max-pages", type=int, default=int(os.getenv("SCRAPER_MAX_PAGES_DEFAULT", "5")))
    parser.add_argument("--out", default=".tmp/raw_listings_bizquest.json")
    ...
    for state in args.states:
        print(f"\n[BizQuest] Scraping {state}...")
        listings = scrape_state(state.upper(), args.max_pages)
        all_listings.extend(listings)
        print(f"[BizQuest] {state}: {len(listings)} listings")
